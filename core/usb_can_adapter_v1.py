...
class UsbCanAdapter:
    ...
    def __init__(self):
        ...
        self.data_dict = {}
        self._fast_seq = 0  # 5bit rolling sequence for fast packet
    ...
    def send_fast_packet(self, can_id: int, payload: bytes) -> int:
        """ 
            Sends an NMEA2k fast packet message (payload => 223 bytes)
            fast packet = multiple packets sent which make up one msg
        """
        ...
        seq = (self._fast_seq + 1) & 0x1F
        self._fast_seq = seq

        # frame one
        frame0 = bytearray(8)
        frame0[0] = (seq << 5) | (total_len & 0x1F)
        frame0[1:8] = payload[0:7].ljust(7, b"\x00")
        if self.send_data_frame(can_id, bytes(frame0), CANUSB_FRAME.EXTENDED) < 0:
            return -1
        ...
        while offset < total_len:
            ...
            frame[1:8] = payload[offset:offset+7].ljust(7, b"\x00")
            if self.send_data_frame(can_id, bytes(frame), CANUSB_FRAME.EXTENDED) < 0:
                return -1
            offset += 7
            frame_index += 1
        return 0
    ...
    def build_data_frame_packet(self, can_id: int, data: bytes, frame_type: Optional[CANUSB_FRAME] = None) -> bytearray:
        """
        Builds a fixed 20-byte "send data frame" packet for the USB-CAN-A adapter.

        Packet format:
        - Header: 0xaa 0x55
        - Type: 0x01 (data)
        - Frame type: 0x01 (standard) / 0x02 (extended)
        - Frame format: 0x01 (data frame)
        - Frame ID: 4 bytes, big-endian
        - Data length: 0..8
        - Data: 8 bytes (padded with zeros)
        - Reserve: 0x00
        - Checksum: low 8 bits of sum of bytes [2..18]
        """
        if frame_type is None:
            if can_id <= 0x7FF:
                frame_type = CANUSB_FRAME.STANDARD
            elif can_id <= 0x1FFFFFFF:
                frame_type = CANUSB_FRAME.EXTENDED
            else:
                raise ValueError("CAN ID out of range (must be <= 0x1FFFFFFF).")

        if len(data) > 8:
            raise ValueError("CAN data too long (max 8 bytes).")

        binary_data = bytearray(8)
        binary_data[:len(data)] = data

        tx_frame = bytearray()
        tx_frame.extend([0xAA, 0x55])
        tx_frame.append(0x01)
        tx_frame.append(frame_type.value)
        tx_frame.append(0x01)
        tx_frame.extend(can_id.to_bytes(4, byteorder="big"))
        tx_frame.append(len(data))
        tx_frame.extend(binary_data)
        tx_frame.append(0x00)
        tx_frame.append(self.generate_checksum(tx_frame[2:19]))
        return tx_frame

    def send_data_frame(self, can_id: int, data: bytes, frame_type: Optional[CANUSB_FRAME] = None) -> int:
        """
        Sends a single CAN data frame (no looping / pacing).

        This is the most efficient entry point to use from other Python code:
        initialize the adapter once, then call this repeatedly.
        """
        if not self.serial_device or not self.serial_device.is_open:
            print("Error: Serial port is not open.")
            return -1
        ...
        try:
            tx_frame = self.build_data_frame_packet(can_id=can_id, data=data, frame_type=frame_type)
        except ValueError as e:
            print(f"Failed to build CAN frame: {e}")
            return -1

        try:
            return self.frame_send(tx_frame)
        except SerialPortError as e:
            print(f"Failed to send CAN frame: {e}")
            return -1

    def send_data_frame_hex(self, hex_id: str, hex_data: str) -> int:
        """Convenience wrapper for sending a single CAN data frame from hex strings."""
        try:
            can_id = int(hex_id, 16)
        except ValueError:
            print("Unable to convert ID from hex to integer!")
            return -1

        payload = self.convert_from_hex(hex_data)
        if len(payload) == 0:
        ...
        return self.send_data_frame(can_id=can_id, data=bytes(payload))
    ...
    def inject_data_frame(self, hex_id: str, hex_data: str) -> int:
        ...
        if can_id <= 0x7FF:
            ...
            print("CAN ID out of range (must be <= 0x1FFFFFFF).")
            return -1
        ...
        if len(payload) == 0:
            ...
            return -1
        if len(payload) > 8:
            print("Data too long (max 8 bytes); truncating.")
            payload = payload[:8]

        data_len = len(payload)
        binary_data = bytearray(8)
        binary_data[:data_len] = payload
        ...
        # Build a fixed 20-byte "send data frame" packet (USB-CAN-A protocol).
        # Header: 0xaa 0x55
        # Type: 0x01 (data)
        # Frame type: 0x01 (standard) / 0x02 (extended)
        # Frame format: 0x01 (data frame)
        ...
        while self.program_running and not error:
            ...
            if self.inject_payload_mode == CANUSB_PAYLOAD_MODE.INJECT_PAYLOAD_MODE_RANDOM:
                ...
                for i in range(data_len):
                    binary_data[i] = (binary_data[i] + 1) & 0xFF

            tx_frame[10:18] = binary_data
            tx_frame[19] = self.generate_checksum(tx_frame[2:19])

            try:
                self.frame_send(tx_frame)
            except SerialPortError as e:
                print(f"Failed to inject frame: {e}")
                return -1
    ...
    def main(self) -> None:
        ...
        if args.n is not None:
            self.terminate_after = args.n

        if args.g is not None:
            self.inject_sleep_gap = args.g

        if args.m is not None:
            try:
                self.inject_payload_mode = CANUSB_PAYLOAD_MODE(args.m)
            except ValueError:
                print("Please specify a valid inject payload mode (0=random, 1=incremental, 2=fixed)!")
                self.display_help()
                sys.exit(1)
        ...
        if self.canusb_int_to_speed(speed) == 0:
            print("Please specify a supported CAN speed!")
            self.display_help()
            sys.exit(1)
